...
/* Softmax / argmax for final 3-class output */
void hw_act_layer3(ap_fixed<32,24> input[1][n_layer3], int &pred){
    ap_fixed<32,24> max_val = input[0][0];
    int max_idx = 0;

    for (int i = 1; i < n_layer3; i++){
        if (input[0][i] > max_val){
            max_val = input[0][i];
            max_idx = i;
        }
    }
    pred = max_idx; // 0 = class 0, 1 = class 1, 2 = class 2
}
...
int nn_inference(float input_img[n_inputs]){
    ap_fixed<32,24> fp_input_img[n_inputs];
    ...
    hw_act_layer3(temp_output3, prediction_int);
